...
export function statementRangeAtCursor(sql: string, cursorPos: number, databaseType?: DatabaseType): SqlTextRange | null {
  ...
  if (databaseType === "mysql" && isCursorOnDelimiterCommandLine(sql, pos)) return null;
  if (isCursorOnBlankLine(sql, pos)) return null;

  const previousDelimitedStatement = statementBeforeCursorInDelimiterGap(sql, statements, pos, databaseType);
  if (previousDelimitedStatement) return previousDelimitedStatement;
...
function isCursorOnDelimiterCommandLine(sql: string, pos: number): boolean {
  const lineStart = sql.lastIndexOf("\n", Math.max(0, pos - 1)) + 1;
  return startsDelimiterCommand(sql, lineStart);
}

function statementBeforeCursorInDelimiterGap(sql: string, statements: RawStatement[], pos: number, databaseType?: DatabaseType): SqlTextRange | null {
  for (let index = 0; index < statements.length; index += 1) {
    const statement = statements[index];
    const next = statements[index + 1];
    const gapEnd = next ? next.from : sql.length;
    if (pos <= statement.to || pos >= gapEnd) continue;
    const between = sql.slice(statement.to, pos);
    const delimiterIndex = between.lastIndexOf(";");
    if (delimiterIndex === -1 || between.slice(delimiterIndex + 1).trim() !== "") continue;
    const softRanges = splitStatementRangeAtSoftStarts(sql, statement, databaseType);
    return rangeForCursorInSoftRanges(sql, softRanges, pos) ?? rangeFor(statement, sql);
  }
  return null;
