...
describe("statementRangeAtCursor", () => {
  ...
  it("returns the previous statement when the cursor is in indentation after its semicolon", () => {
    const sql = "SELECT 1;\n    SELECT 2;";
  ...
  it("returns the next statement when the cursor reaches its SQL text", () => {
    const sql = "SELECT 1;\n    SELECT 2;";
    const statementPos = sql.indexOf("SELECT 2");
    const range = statementRangeAtCursor(sql, statementPos);
    expect(range?.sql.trim()).toBe("SELECT 2");
  });
...
describe("buildExecutionCandidates", () => {
  ...
  it("returns previous + all when the cursor is in indentation after a semicolon", () => {
    const sql = "SELECT 1;\n    SELECT 2;";
    const indentationPos = sql.indexOf("    SELECT 2") + 2;
